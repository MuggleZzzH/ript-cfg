...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        Args:
            ...
            load_state (bool): whether to load simulator state from the dataset
        """
        ...
        # print('\tto_load_next_obs', self.load_next_obs)
        ...
        # print('self.obs_keys', self.obs_keys)
    ...
    def get_dataset_for_ep(self, ep, key):
        ...
        if key_should_be_in_memory and key != 'states':
            # read cache
            if '/' in key:
                key1, key2 = key.split('/')
                assert(key1 in ['obs', 'next_obs'])
    ...
    def get_item(self, index):
        ...
        if self.load_next_obs:
            ...
            # changing next_obs to be densely sampled observations for all the frames in the sequence
            meta["next_obs"] = self.get_obs_sequence_from_demo(
                demo_id,
                index_in_demo=index_in_demo,
                keys=self.obs_keys,
        ...
        if self.load_state:
            meta["init_state"] = self.get_dataset_sequence_from_demo(
                demo_id,
                index_in_demo=index_in_demo,
                keys=['states'],
                seq_length=1,
